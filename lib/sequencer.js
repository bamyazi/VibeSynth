...
function generateRandomPattern() {
  ...
  const numVoices = Math.floor(Math.random() * 3) + 1;
  ...
  for (let voiceIdx = 0; voiceIdx < numVoices; voiceIdx++) {
    ...
    for (let i = 0; i < notesPerBar; i++) {
      ...
      if (Math.random() < density) {
        ...
        voicePattern[voiceIdx][noteIdx][step] = {
          active: true,
          length: length,
          accent: accent,
          slide: slide
  ...
  // Random mixer settings
  for (let v = 0; v < 3; v++) {
    if (voiceSettings[v]) {
      // Random waveform
      const waveforms = ['square', 'sawtooth', 'triangle', 'pulse'];
      voiceSettings[v].waveType = waveforms[Math.floor(Math.random() * waveforms.length)];
      
      // Random octave (-2 to 1)
      voiceSettings[v].octave = Math.floor(Math.random() * 4) - 2;
    }
  }
  
  // Refresh UI
  updateVoiceGridDisplay();
  updateDrumGridDisplay();
  
  console.log('Random pattern generated with scale:', scaleNames[scaleNames.indexOf(Object.keys(scales).find(k => scales[k] === chosenScale))]);
}
